    ...
    <>
      <AnimatePresence>
        {/* {visible && ( */}
          ...
          >
            <div
              className={`glass-nav flex items-center gap-1 px-3 py-2 rounded-full border ${
                isDark
            ...
            >
              {/* Logo */}
              <span
                className={`font-semibold text-sm mr-3 pl-2 cursor-pointer font-space ${
              ...
              <div className="flex items-center gap-1.5 ml-auto">
                  ...
                  <AnimatePresence>
                    {themeMenuOpen && (
                      <motion.div
                        ...
                        className={`absolute right-0 top-full mt-2 rounded-2xl border overflow-hidden shadow-xl ${
                          isDark ? 'bg-[#1a2234] border-white/10' : 'bg-white border-black/8'
                        }`}
                          ...
                          <button
                            ...
                            className={`w-full flex items-center gap-2.5 px-4 py-2.5 text-[13px] font-medium transition-colors duration-150 ${
                              theme === opt.value
                                ...
                                : isDark ? 'text-gray-400 hover:text-white hover:bg-white/5' : 'text-gray-500 hover:text-gray-900 hover:bg-black/3'
                            }`}
                          >
                            {opt.icon}
                            {opt.label}
                ...
                <button
                  ...
                  className={`md:hidden p-1.5 rounded-full transition-colors duration-150 ${
                    isDark ? 'text-gray-400 hover:text-white hover:bg-white/8' : 'text-gray-500 hover:text-gray-900 hover:bg-black/5'
                  }`}
                >
            ...
            <AnimatePresence>
              {mobileOpen && (
                <motion.div
                  ...
                  className={`absolute top-full mt-2 left-4 right-4 rounded-2xl border p-3 glass-nav ${
                    isDark ? 'bg-[#111827]/90 border-white/10' : 'bg-white/90 border-black/8 shadow-xl'
                  }`}
                  style={{ maxWidth: '660px', margin: '8px auto 0' }}
                >
                  {navLinks.map(link => (
                    <button
                      key={link.href}
